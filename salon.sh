...
SERVICE_MENU () {
  ...
  # list services –– format: 1) service
...
PHONE_MENU () {
  ...
  if [[ -z $CUSTOMER_NAME ]]
    then
      ...
      read CUSTOMER_NAME
      # enter phone and name into customers table
...
BOOK_TIME () {
  ...
  # add info to appointments table
  APPOINTMENT_BOOKED=$($PSQL "INSERT INTO appointments(customer_id, service_id, time) VALUES($CUSTOMER_ID, $SERVICE_ID_SELECTED, '$SERVICE_TIME')")
  if [[ $APPOINTMENT_BOOKED = "INSERT 0 1" ]]
    then
